...
const AnalysisPage: React.FC = () => {
  ...
  const getScoreTooltip = (scoreType: string) => {
    const tooltips: Record<string, { title: string; description: string; improvement: string }> = {
      savingsScore: {
        title: 'Savings Score',
        description: 'Measures your monthly savings rate. Target: 15-20% of income',
        improvement: 'Increase by reducing expenses or growing income'
      },
      diversityScore: {
        title: 'Spending Diversity',
        description: 'How balanced your spending is across categories',
        improvement: 'Avoid over-concentration in one category'
      },
      impulseScore: {
        title: 'Impulse Control',
        description: 'Measures small, unplanned purchases',
        improvement: 'Reduce spontaneous small purchases (<₪50)'
      },
      runwayScore: {
        title: 'Financial Runway',
        description: 'How long your savings can sustain current spending',
        improvement: 'Build emergency fund (target: 90+ days)'
      }
    };
    return tooltips[scoreType] || { title: scoreType, description: '', improvement: '' };
  };
  ...
  return (
    <Box>
      ...
      {/* Actionability Setup Modal */}
      <ActionabilitySetupModal
        open={setupModalOpen}
        onClose={() => setSetupModalOpen(false)}
        onSave={() => {
          // Optionally refresh intelligence data after saving settings
          fetchIntelligence();
        }}
      />
          ...
          <Grid item xs={12} md={8}>
            <Grid container spacing={2}>
              {Object.entries(healthBreakdown).map(([key, value]) => {
                const tooltip = getScoreTooltip(key);
                return (
                  <Grid item xs={6} key={key}>
                    <Tooltip
                      title={
                        <Box>
                          <Typography variant="subtitle2" gutterBottom>{tooltip.title}</Typography>
                          <Typography variant="body2" gutterBottom>{tooltip.description}</Typography>
                    ...
                    >
                      <Box>
                        <Typography variant="body2" color="text.secondary" gutterBottom sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                          ...
                          <InfoIcon sx={{ fontSize: 14, opacity: 0.5 }} />
                        </Typography>
