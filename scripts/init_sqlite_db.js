...
function rebuildInvestmentHoldingsForPikadonEntries(db) {
  const columns = getTableColumns(db, 'investment_holdings');
  ...
  const tableSql = getTableSql(db, 'investment_holdings');
  const needsRebuild = tableSql.includes('UNIQUE(account_id, as_of_date)');
  if (!needsRebuild) {
    return;
  }
...
const TABLE_DEFINITIONS = [
  ...
  `CREATE TABLE IF NOT EXISTS optimizer_v2_review_groups (
      group_key TEXT PRIMARY KEY CHECK(group_key IN ('household', 'cash_flow', 'banking', 'investments', 'real_estate')),
  ...
  `CREATE TABLE IF NOT EXISTS optimizer_v2_candidates (
      ...
      lifecycle_state TEXT NOT NULL DEFAULT 'candidate' CHECK(lifecycle_state IN ('candidate', 'added', 'started', 'snoozed', 'done', 'dismissed')),
      ...
      outcome_band TEXT CHECK(outcome_band IS NULL OR outcome_band IN ('none', 'below_estimate', 'within_estimate', 'above_estimate', 'unknown')),
  ...
  `CREATE TABLE IF NOT EXISTS optimizer_v2_sources (
      ...
      valid_until TEXT,
      verified_at TEXT,
      availability_status TEXT NOT NULL DEFAULT 'unverified' CHECK(availability_status IN ('unverified', 'available', 'unavailable')),
      created_at TEXT NOT NULL DEFAULT (datetime('now')),
      FOREIGN KEY (run_id) REFERENCES optimizer_v2_runs(id) ON DELETE CASCADE,
  ...
  `CREATE TABLE IF NOT EXISTS action_item_history (
      ...
      action TEXT NOT NULL CHECK(action IN ('created', 'dismissed', 'resolved', 'snoozed', 'reactivated', 'updated', 'accepted', 'completed', 'failed')),
...
const INDEX_STATEMENTS = [
  ...
  'CREATE INDEX IF NOT EXISTS idx_optimizer_v2_review_status ON optimizer_v2_review_groups (status);',
  'CREATE INDEX IF NOT EXISTS idx_optimizer_v2_runs_generated ON optimizer_v2_runs (generated_at DESC);',
  'CREATE INDEX IF NOT EXISTS idx_optimizer_v2_candidates_run_score ON optimizer_v2_candidates (run_id, score DESC);',
  'CREATE INDEX IF NOT EXISTS idx_optimizer_v2_candidates_lifecycle ON optimizer_v2_candidates (lifecycle_state);',
  'CREATE INDEX IF NOT EXISTS idx_optimizer_v2_sources_run ON optimizer_v2_sources (run_id);',
