...
function ensureEnvKeyForLinuxDev() {
  const directEnvKey = process.env.SHEKELSYNC_ENCRYPTION_KEY;
  if (directEnvKey) {
    if (!isKeyValid(directEnvKey)) {
      throw new Error(
        'SHEKELSYNC_ENCRYPTION_KEY is set but invalid. Expected a 64-character hex string.',
      );
    }
    return directEnvKey;
  }
  ...
  if (fs.existsSync(appEnvLocalPath)) {
    ...
    const fileKey = readEnvValue(existingContent, 'SHEKELSYNC_ENCRYPTION_KEY');
    if (fileKey) {
      if (!isKeyValid(fileKey)) {
        throw new Error(
          `Invalid SHEKELSYNC_ENCRYPTION_KEY in ${appEnvLocalPath}. Expected a 64-character hex string.`,
        );
      }
      return fileKey;
    }
  ...
  const generatedBlock =
    ...
    `SHEKELSYNC_ENCRYPTION_KEY=${generatedKey}\n`;
