...
export interface DonationStatusContextValue {
...
const DonationStatusContext = createContext<DonationStatusContextValue | undefined>(undefined);
...
export const DonationStatusProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Listen for status change events (from mutations in this provider)
  useEffect(() => {
  ...
  const value: DonationStatusContextValue = useMemo(() => ({
    status,
    loading,
    error,
    ...
    addDonationEvent,
    markReminderShown,
  }), [status, loading, error, derived, refresh, createSupportIntent, addDonationEvent, markReminderShown]);

  return (
    <DonationStatusContext.Provider value={value}>
      {children}
    </DonationStatusContext.Provider>
  );
};

export function useDonationStatusContext(): DonationStatusContextValue {
  const context = useContext(DonationStatusContext);
  if (!context) {
    throw new Error('useDonationStatusContext must be used within a DonationStatusProvider');
  }
  return context;
}
